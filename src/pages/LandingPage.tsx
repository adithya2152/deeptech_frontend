...
export default function LandingPage() {
  ...
  const engagementModels = [
    ...
    },
    {
      icon: Target,
      title: 'Fixed Price',
      description: 'Perfect for well-defined deliverables. Set a scope, agree on a price, and get it done.',
      color: 'text-blue-500',
      bg: 'bg-blue-500/10',
    ...
    },
    {
      icon: CalendarDays,
      title: 'Daily Rate',
      description: 'Flexible R&D and problem solving. Pay for expert time on a day-by-day basis.',
  ...
  const features = [
    ...
    },
    {
      ...
      description: 'Escrow-backed payments released only when specific technical milestones are met and verified.',
    },
  ];
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {engagementModels.map((model, idx) => (
              <Card key={idx} className="relative overflow-hidden hover:shadow-lg transition-all duration-300 border-border/50 group">
                ...
                <CardHeader>
                  <div className={`h-12 w-12 rounded-lg flex items-center justify-center mb-4 ${model.bg} ${model.color}`}>
            ...
            {domains.map(([key, label]) => (
              ...
              >
                <CardContent className="flex flex-col items-center justify-center p-5 space-y-4">
                  <p className="font-semibold text-sm group-hover:text-primary transition-colors text-center">{label}</p>
                </CardContent>
              </Card>
            ))}
          </div>
