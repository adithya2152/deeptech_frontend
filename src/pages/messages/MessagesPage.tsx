    ...
    <Layout>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 text-center md:text-left">
          ...
          <p className="text-muted-foreground mt-1">Communicate with your project partners</p>
        </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        {conversation.unreadCount > 0 && (
                          <div className="h-5 w-5 rounded-full bg-primary flex items-center justify-center shrink-0">
                            <span className="text-[10px] text-primary-foreground font-bold">{conversation.unreadCount}</span>
                          </div>
                        )}
                      </div>
                    </button>
          ...
          <Card className="md:col-span-2 overflow-hidden flex flex-col h-full shadow-md">
            {selectedConversation ? (
              <div className="flex flex-col h-full">
                <div className="p-4 border-b flex items-center justify-between bg-card z-10 shadow-sm">
                  <div className="flex items-center gap-3">
                    <Avatar className="h-10 w-10 border">
                      <AvatarImage src={selectedConversationData?.otherUser.avatar_url} />
                      <AvatarFallback className="bg-muted text-muted-foreground">
                        {getInitials(selectedConversationData?.otherUser.name || '')}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      <p className="font-semibold text-sm">{selectedConversationData?.otherUser.name}</p>
                      <p className="text-xs text-muted-foreground capitalize">{selectedConversationData?.otherUser.role}</p>
                    </div>
                  </div>
                ...
                <ScrollArea ref={scrollAreaRef} className="flex-1 p-4 bg-muted/5">
                  {loadingMessages ? (
                ...
                <div className="p-4 bg-background border-t">
                  ...
                  >
                    <Input
                      ...
                      autoComplete="off"
                    />
                    <Button type="submit" size="icon" className="h-11 w-11 shrink-0" disabled={!messageText.trim() || sendMessage.isPending}>
              ...
              <div className="flex flex-col items-center justify-center h-full text-center p-8 bg-muted/5">
                ...
                <p className="text-muted-foreground max-w-xs mt-2 text-sm">
                  Select a conversation from the list to view your messages and start chatting.
                </p>
              </div>
            )}
          </Card>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently remove the chat history for both you and the other participant.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
