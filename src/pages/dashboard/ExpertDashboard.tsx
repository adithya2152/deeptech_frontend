...
export function ExpertDashboard() {
    ...
    const completedContracts = contracts?.filter((c: any) => c.status === 'completed').length || 0;
                ...
                <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    {!isProfileComplete && (
                                        <Badge variant="destructive" className="cursor-pointer" onClick={() => navigate('/profile')}>
                                            Complete Profile
                                        </Badge>
                                    )}
                                </div>
                        ...
                        <div className="flex gap-3">
                            <Button variant="outline" onClick={() => navigate('/proposals')} disabled={!isProfileComplete}>
                                My Proposals
                            </Button>
                            <Button onClick={() => navigate('/marketplace')} className="bg-slate-900 hover:bg-slate-800" disabled={!isProfileComplete}>
            ...
            <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
                ...
                {!isProfileComplete && (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-semibold text-amber-900">Your profile is incomplete</h3>
                                <p className="text-sm text-amber-700">Complete your expertise profile to get verified and start bidding on projects.</p>
                            </div>
                        </div>
                        <Button size="sm" onClick={() => navigate('/profile')} className="bg-amber-600 hover:bg-amber-700 text-white border-none">
                            Complete Now
                        </Button>
                    </motion.div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card className="col-span-1 md:col-span-2 shadow-sm border-slate-200">
                        <CardHeader>
                            <CardTitle className="text-lg font-medium text-slate-700">Earnings Overview</CardTitle>
                        </CardHeader>
                        <CardContent>
                ...
                <div>
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Button variant="ghost" onClick={() => navigate('/marketplace')} disabled={!isProfileComplete}>View all</Button>
                    </div>
                    ...
                    {!isProfileComplete ? (
                        <Card className="border-dashed bg-slate-50">
                            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                                ...
                                <h3 className="font-semibold text-lg text-slate-900 mb-2">Marketplace Locked</h3>
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {marketplace_projects?.slice(0, 3).map((project: any) => (
                                <Card key={project.id} className="hover:shadow-md transition-shadow cursor-pointer group" onClick={() => navigate(`/projects/${project.id}`)}>
                                    <CardHeader className="pb-3">
                                        <div className="flex justify-between items-start">
                                            <Badge variant="outline" className="mb-2 bg-slate-50">{project.project_type || 'Project'}</Badge>
