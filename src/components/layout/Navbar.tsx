...
export function Navbar() {
  const { user, profile, isAuthenticated, token, logout, switchRole } =
    useAuth();
                ...
                {isExpert && (
                  <>
                    ...
                    >
                      ...
                      <NavBadge
                        count={
                          notifCounts?.invitations ?? notifCounts?.proposals
                        }
                      />
                    </button>
                    <button
            ...
            {isAuthenticated ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="relative h-10 w-10 rounded-full p-0 overflow-hidden ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 hover:bg-muted"
                  >
                    <Avatar
                      key={avatarUrl}
                ...
                <DropdownMenuContent className="w-64" align="end">
                      ...
                      <div className="p-2">
                        ...
                        <div
                          ...
                          onClick={() =>
                            !isBuyer &&
                            !isSwitching &&
                            handleConfirmSwitchRole()
                          }
                        >
                          <div className="flex items-center gap-2">
                            <div
                              className={`h-7 w-7 rounded-full flex items-center justify-center ${isBuyer ? "bg-emerald-100" : "bg-zinc-100"}`}
                            >
                              {isSwitching && !isBuyer ? (
                                <Loader2 className="h-3.5 w-3.5 text-zinc-500 animate-spin" />
                              ) : (
                                <Briefcase
                                  className={`h-3.5 w-3.5 ${isBuyer ? "text-emerald-600" : "text-zinc-500"}`}
                                />
                              )}
                            </div>
                            <span
                              className={`text-sm font-medium ${isBuyer ? "text-emerald-700" : "text-muted-foreground"}`}
                            >
                              {isSwitching && !isBuyer
                                ? "Switching..."
                                : "Buyer"}
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <div
                              className={`h-7 w-7 rounded-full flex items-center justify-center ${isExpert ? "bg-emerald-100" : "bg-zinc-100"}`}
                            >
                              {isSwitching && !isExpert ? (
                                <Loader2 className="h-3.5 w-3.5 text-zinc-500 animate-spin" />
                              ) : (
                                <User
                                  className={`h-3.5 w-3.5 ${isExpert ? "text-emerald-600" : "text-zinc-500"}`}
                                />
                              )}
                            </div>
                            <span
                              className={`text-sm font-medium ${isExpert ? "text-emerald-700" : "text-muted-foreground"}`}
                            >
                              {isSwitching && !isExpert
                                ? "Switching..."
                                : "Expert"}
                  ...
                  <DropdownMenuItem
                    disabled={isExpert && !canUseExpertApp}
                    onClick={() => canUseExpertApp && navigate("/messages")}
                  >
            ...
            {isAuthenticated ? (
              <>
                <div className="flex items-center gap-3 px-2 py-3 mb-2 bg-muted/50 rounded-xl">
                  <Avatar
                    key={avatarUrl}
                    className="h-12 w-12 border border-border shadow-sm"
                  >
                    <AvatarImage src={avatarUrl} className="object-cover" />
                ...
                {!isAdmin && (
                  <div className="mb-3">
                    ...
                    <div
                      ...
                      onClick={() =>
                        !isBuyer && !isSwitching && handleConfirmSwitchRole()
                      }
                    >
                      <div className="flex items-center gap-2">
                        <div
                          className={`h-8 w-8 rounded-full flex items-center justify-center ${isBuyer ? "bg-emerald-100" : "bg-zinc-100"}`}
                        >
                          {isSwitching && !isBuyer ? (
                            <Loader2 className="h-4 w-4 text-zinc-500 animate-spin" />
                          ) : (
                            <Briefcase
                              className={`h-4 w-4 ${isBuyer ? "text-emerald-600" : "text-zinc-500"}`}
                            />
                          )}
                        </div>
                        <span
                    ...
                    <div
                      ...
                      onClick={() =>
                        !isExpert && !isSwitching && handleConfirmSwitchRole()
                      }
                    >
                      <div className="flex items-center gap-2">
                        <div
                          className={`h-8 w-8 rounded-full flex items-center justify-center ${isExpert ? "bg-emerald-100" : "bg-zinc-100"}`}
                        >
                          {isSwitching && !isExpert ? (
                            <Loader2 className="h-4 w-4 text-zinc-500 animate-spin" />
                          ) : (
                            <User
                              className={`h-4 w-4 ${isExpert ? "text-emerald-600" : "text-zinc-500"}`}
                            />
                          )}
                        </div>
                        <span
                ...
                {isExpert && (
                  <>
                    <Link
                      to="/marketplace"
                      className={`flex items-center gap-2 p-3 rounded-lg hover:bg-muted text-sm font-medium ${!canUseExpertApp ? "opacity-50 pointer-events-none" : ""}`}
                      onClick={() => setMobileMenuOpen(false)}
                    >
                ...
                <button
                  ...
                  className="flex items-center gap-2 p-3 rounded-lg hover:bg-destructive/10 w-full text-destructive text-sm font-bold"
                >
              ...
              <>
                <Link
                  to="/experts"
                  className="block p-3 rounded-lg hover:bg-muted text-sm font-medium"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {"Browse Experts"}
                </Link>
